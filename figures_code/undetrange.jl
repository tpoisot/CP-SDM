...
ax2 = Axis(f[1, 1], xlabel="Risk level", ylabel="Inefficiency")
hlines!(ax2, [1.0], color=:grey50, linestyle=:dash)
x, m, s = _agr(risk_levels, eff)
errorbars!(ax2, x, m, s, color=:black)
scatterlines!(ax2, x, m, color=:black)

ax3 = Axis(f[2, 1], xlabel="Risk level", ylabel="Undetermined range (km²)", yscale=log10)
surf_undet[findall(iszero, surf_undet)] .= NaN
x, m, s = _agr(risk_levels, surf_undet)
errorbars!(ax3, x, m, s, color=:black)
scatterlines!(ax3, x, m, color=:black)
xlims!(ax2, extrema(risk_levels))
xlims!(ax3, extrema(risk_levels))
Label(f[1, 1], " A ", alignmode=Inside(), tellwidth=false, tellheight=false, valign=:top, halign=:left, fontsize=30)
Label(f[2, 1], " B ", alignmode=Inside(), tellwidth=false, tellheight=false, valign=:bottom, halign=:left, fontsize=30)
