...
The ability to predict where species may be found is a cornerstone of biogeography and macroecology  #cite(<franklin2023species>). Techniques from the field of applied machine learning (ML hereafter) are now routinely used alongside ecological approaches to train generalizable species distribution models (SDMs hereafter) #cite(<beery2021species>). SDMs generate a binary response (corresponding to the prediction that the species is likely present/absent under given environmental conditions) or a quantitative score most often as a probability of presence or habitat suitability, indicating how strongly we believe that the species may be present at the location.

Proper communication of the uncertainty associated to the prediction of a SDM is important, since we usually seek to apply these models to look both forward and backwards in time. This projection if the model to different times is usually called "transfer" #cite(<zurell2012predicting>), whereby a model trained under historical (baseline) conditions is applied to past/future projections of the same predictors. The projection of SDMs can also happen in space #cite(<petitpierre2016selecting>), to predict where species may invade or be naturalized. Even when predictions are not projected, spatial knowledge of the uncertainty is valuable information: it can be used to identify areas where the model predictions are trustworthy. Current checklists on the reproductibility of SDMs emphasize the consequences of data uncertainty #cite(<feng2019checklist>). Yet, predictions also have inherent uncertainty, which is usually not adequately communicated. This can be, for example, because of genuine uncertainty about (or inability to capture through the model) the actual response of the species to combination of predictors #cite(<parker2024insufficient>).

A common way to capture information about the variability of SDMs is to rely on non-parametric bootstrapping #cite(<valavi2021modelling>), wherein models trained on random subsets of the data are compared to estimate the distribution of the response under incomplete sampling. This approach captures more than one type of variability #cite(<thuiller2019uncertainty>), and provide valuable information about the range of performances that can be expected from a model. Other methods are built into the predictor itself, as is the case for _e.g._ BARTs #cite(<carlson2020embarcadero>), which estimate their own uncertainty. But either situation comes with drawbacks. Bootstrapping requires to train and evaluate the model hundreds of times, and on partial datasets, which is computationally inefficient. Using #change[methods that are specific to a particular classification algorithm] limits one to the classifier for which these methods are available, which prevents for example the use of a new algorithm with the same estimation of uncertainty.

In this manuscript, I illustrate how the ML technique of conformal prediction (CP) allows to identify instances (combinations of environmental variables) for which a trained and calibrated model cannot confidently make predictions #cite(<gammerman1998learning>). A brief introduction to CP is provided in this manuscript, but the topic is covered in more depth by #cite(<shafer2007tutorial>, form: "prose") for the mathematical foundations, by #cite(<fontana2020conformal>, form: "prose") for a historical perspective, and by #cite(<angelopoulos2023conformal>, form: "prose") for concrete recommendations. By way of contrast to _e.g._ bootstrapping, CP does not necessarily involve retraining the same model many times over, but instead wraps the model into an additional prediction step, and returns estimates of credibility based on the distribution of past model predictions compared to ground-truthed data. This is an important difference, as #change[conformal prediction makes no assumption about the distribution of data, but rather captures the uncertainty associated to the distribution of observed model outcomes] #cite(<lei2013distributionfree>). Conformal prediction provides what is essentially (for classification problems) a confidence interval around the presence or absence of a species in a given location. This is a particularly important feature, in that CP achieves this in a way that creates several analogues between ML prediction and fundamental concepts in frequentist statistics #cite(<neyman1937outline>).

One of the reasons why CP is particularly promising for uncertainty quantification in SDMs is that it is a distribution-free method: it requires neither assumptions about the model nor prior knowledge of the outcome distribution to provide confidence intervals that are as small as possible while being _guaranteed_ to contain the true value under a set risk level #cite(<vovk2018crossconformal>). This is particularly important when transferring a SDM to novel environments #cite(<zurell2012predicting>), where we expect covariate shift (the joint distributions of predictors are different when training and predicting), a prediction context that CP is robust to #cite(<fannjiang2022conformal>) #cite(<tibshirani2019conformal>).
...
One of the most efficient ways to increase the performance of binary classifiers is to change the decision rule leading to a positive (here, presence) prediction, so that presences are assigned when $p_+ >= tau$ – a process known as moving threshold classification #cite(<liu2016selection>) #cite(<liu2013selecting>). The value of $tau$ is an hyper-parameter of the model, which is chosen to maximize the value of a measure of model performance (here the MCC) when evaluated over many different values. In this instance, we optimized the value of $tau$ by picking the value out of 200 linearly spaced value between the smallest and largest prediction made on the training set. The value of $tau$ that maximizes the MCC during cross-validation was selected as the optimal threshold for the classifier. Note that even though our decision rule for the presence of the species is $p_+ >= tau$, we will keep the information about $p_-$ as is it required for conformal prediction.

=== Bootstrap variability

Bagging (bootstrap aggregating) is often used as a measure of uncertainty to the underlying data when training SDMs #cite(<beale2012incorporating>). When performing bagging, the model is trained on samples drawn with replacement from the training set (which leaves out approx. 37% of the dataset). Models are then evaluated on samples that were not used as part of their training, usually using cross-validation #cite(<bylander2002estimating>) or measures of the out-of-bag error #cite(<janitza2018overestimation>). Although ensemble models _can_ result in a better predictive performance compared to single models #cite(<drake2014ensemble>), this is not a guarantee (and depends on the structure of the bias/variance trade-off for the specific model and its training set). The many models trained on the bagging dataset form an homogeneous ensemble, which is to say a set of models that share the same algorithm and hyper-parameters, and only make different predictions as the result of having been trained on different subsets of the full training set.

Measures of whether the different models composing the homogeneous ensemble agree can provide a measure of the effect of data and parameter uncertainty #cite(<petropoulos2018exploring>), or what #cite(<davies2023addressing>, form: "prose") termed the "SDM uncertainty". The best model identified after thresholding was evaluated on a hundred bootstrap samples, yielding an homogeneous ensemble model from which we estimate bootstrap variability #cite(<chen2019uncertainty>). Because the model is kept constant in this analysis, the measure of variability we will derive from the ensemble model is an estimate of how sensitive the estimation of the model parameters is to small perturbations (specifically: spatially homogeneous under-sampling) to the training data.

= An introduction to conformal prediction

Conformal prediction differs from regular prediction in that, rather than a single point prediction, it returns sets corresponding to the ensemble of _credible_ outcomes given an input $bold(x)$ representing environmental conditions at which we seek to make the prediction. Given the observed quantiles of the model output on validation data, these sets are obtained through a simple calibration step. Therefore, CP requires an already trained model, and is agnostic to the process through which this model is trained. In this section, I highlight two important features of CP: the notion of _#change[prediction sets]_ (and how they are obtained), and the notion of _coverage_ , which is a measure of tolerance to error.

== Understanding conformal predictions

By contrast to the non-conformal SDM, the conformal classifier returns, for an input of environmental predictors $bold(x)$, a set $C$ containing the "credible outcomes" for this prediction. This set is termed the _#change[prediction set]_, and under a binary classification task (the species is either present or absent), there are four possible combinations for the content of #change[prediction sets]: $C = {+}$, $C = {-}$, $C = {+, -}$, and $C = emptyset$.

The first two cases are simple: if the #change[prediction set] contains a single output, the model can confidently make a prediction that excludes the other class. In the case of $C = {+}$, for example, the point prediction for the presence score $p_+$ is high enough that the outcome of absence can be ruled out given the known predictions on training examples. In some cases, the #change[prediction set] may contain both classes, as in $C = {+, -}$. Although they may not be _equally likely_ (there is no guarantee that $p_+ approx p_-$), the scores are close enough to not confidently exclude one of the outcomes from the model prediction. In the specific cases of SDMs, these correspond to areas of true uncertainty, where the known training examples credibly support both the presence or absence of the species. The final situation, $C = emptyset$, corresponds to pathological cases where _neither_ outcome can be credibly supported. Given the training data (and the distribution of presences and absences), the model is not able to make a prediction for this input. The increased frequency of such predictions is most likely a strong sign that the risk level is too high (#change[equivalent to a too broad confidence interval]) for the training data given to the conformal model.
...
There are several ways to decide whether a point prediction from the model results in which #change[prediction set]. A core assumption of CP is that the data used for training should be exchangeable, or in other words, their joint probability distribution should be (close to) invariant under finite permutations #cite(<aldous1985exchangeability>). This will almost never be the case for data with a spatial structure; nevertheless, this does not rule out the use of CP for species distribution modeling, as #cite(<oliveira2024split>, form: "prose") show that CP is acceptably robust to lack of exchangeability. #add[The purpose of this section is to establish a general overview of how conformal predictions are obtained, and some of the multiple variations that exist will be introduced throghout the text.]

The central idea of CP is to associate a conformal score to a point prediction. This can be achieved by applying the $text("softmax")$ function to the values for $p_+$ and $p_-$ #add[(note that the values of $p$ are bounded, and proportional to the true event probability)], giving

$
s_+ = (exp p_+)/(exp p_+ + exp (1- p_+)), s_- = (exp (1 - p_+))/(exp p_+ + exp (1- p_+))
$ <equation-softmax>

The conformal score associated to a prediction is $1 - s_dot$, where $dot$ is the prediction ($+$ or $-$) made by the model. We call the distribution of conformal scores $cal(S)$. Note that this can be done without using the $text("softmax")$ function #add[(_i.e._ $s_+ = p_+$, $s_- = 1 - p_+$)], but it is #change[used] here as it is best practice for classification #add[#cite(<dey2023conformal>)]. #add[The use of the softmax function is appropriate here because not all algorithms for species distribution models will return well-calibrated, probabilities, even though $0 <= p_+ <= 1$ and $p_- = 1 - p_+$.]

The next step is to identify a critical value $accent(q, hat)$ above which a conformal score indicates that the prediction it describes is credible. This critical value is picked by examining the empirical quantile distribution of the conformal scores #add[in the distribution $cal(S)$] calculated over $n$ training examples, and an acceptable level of risk $alpha$ (explained in depth in the next sub-section). #change[Specifically, this is done by identifying the $q_i$-th quantile of the distribution of model scores], where 

$
  q_i = ceil((n+1)(1-alpha))/n 
$ <equation-quantile>

The corresponding value of $S$ below which a proportion $q_i$ of values lies is $accent(q, hat)$. In other, more intuitive words, the value $q_i$ indicates what proportion of wrong classification events we must accept before we have accumulated enough evidence to be confident about a prediction. When performing the prediction, we calculate the score of a new prediction according to #ref(<equation-softmax>). For every possible class $x$, if $s_x >= (1 - accent(q, hat))$, this class is retained as part of the #change[prediction set]. #add[Note that some approaches to conformal prediction, some of which will be discussed in the following sections, keep the distribution of scores separate for each class, _i.e._ $cal(S)+-$ and $cal(S_-)$, in which case the quantiles are also class-specific rather than global.]

The value of $accent(q, hat)$ can be obtained either through using a holdout set for training (Split Conformal Prediction), #add[using adaptive prediction sets #cite(<angelopoulos2023conformal>)], by retraining the model in a way aking to Leave-One-Out cross-validation (Full Conformal Prediction), through the use of quantile regression #cite(<romano2019conformalized>), or through taking the median of several estimates of $accent(q, hat)$ after cross-validation #cite(<vovk2018crossconformal>). #cut[In this manuscript, I employ the later method, as it provides a rapid and statistically acceptable estimate of $accent(q, hat)$, without requiring too much computing time.]

To summarize, the output of the conformal classifier is, in a sense, a point estimate of the credible outcomes of a model, using the value estimated for $p_+$ as well as knowledge about which of these were associated to the correct label in the training data. A location is defined as included in the range is the positive outcome is included within the #change[prediction set] returned by the conformal classifier, and as excluded from the range when it is not. Because the conformal classifier can identify that both outcomes are credible based on the training data (while giving them different weights), predictions in which both the positive and negative outcomes are included in the #change[prediction set] can be seen as "uncertain" at this given risk level.

How frequently a specific prediction is uncertain is termed the inefficiency of the classifier, which is defined as the average cardinality of all #change[prediction sets]. The inefficiency is bounded upwards by the number of classes (two for binary classification); when the inefficiency is $approx 1$, the conformal classifier behaves (essentially) like deterministic classifier, by returning a single class for each instance. An inefficiency close to unity is not desirable: smaller sets can hide our actual uncertainty #cite(<sadinle2018least>). Because the conformal models wraps the logisitc regression model, we can further divide the "unsure" predictions as a function of whether they would be within the range as predicted by the SDM, which I will call "unsure presences"; the other unsure predictions are referred to as "unsure absences".

== The coverage level

CP allows users to set a desired error rate, $alpha$, which appeared in #ref(<equation-quantile>). Intuitively, what CP does, is inform the user on whether the #change[prediction set] contains the true value with probability $1-alpha$, which allows to directly interpret this value as a true confidence interval. This error rate is usually referred to as the _marginal coverage_, in that it captures the probability of success marginalized over the known validation points. Because the estimate of uncertainty involves the original model, it is important to apply CP on a model with adequate performance.

#add[coverage is a well-defined, classical property of confidence intervals in statistics]

Chaning the risk level $alpha$ leads to different estimates of how commonly multiple classes will be accepted as a credible outcome. Using a low level of risk ($alpha approx 0$) yields usually leads to all outcomes being credible ($accent(q, hat) approx 1$), at the cost of a very high uncertainty. When values of $alpha$ get too large ($accent(q, hat) approx 0$), no class can be confidently predicted, and the model will eventually always return $C = emptyset$. Although this later situation is more difficult to make sense of intuitively, a value of inefficiency that gets smaller than unity should be interpreted as a model that accumulates more uncertainty (at a given risk level) than the data can support #cite(<romano2020classification>). Conformal prediction can therefore inform us on the acceptable risk levels we can operate under given a trained predictive model.

In the rest of this analysis, I will set $alpha = 0.05$. As noted by #cite(<angelopoulos2023conformal>, form: "prose"), this corresponds to estimating whether a specific prediction falls within, or outside of, the 95% confidence interval across all predictions, which is a convenient callback to frequentist statistics' usual risk tolerance. Recall that the CP #change[prediction sets] are estimated based on the model output, and therefore even when aiming for full coverage, there may be non-ambiguous combinations of environmental predictors.
...
caption: [Overview of measures of model performance for the validation and training sets of the SDM, as well as the same measures for the ensemble model (measured on the out-of-bag models only). The values of $kappa$ and the true-skill statistic are generally comparable to the MCC, but are included as they are commonly reported in the SDM litterature #cite(<allouche2006assessing>). The high values of the negative and positive predictive values indicate that the model is suitable to detect both presences and absences. #add[NPV and PPV are, respectively, the negative and positive predictive values, which indicate the ability of the classifier to make reliable predictions for the negative and positive outcomes.]],
placement: auto,
) <table-performance>
...
#figure(
  ...
  caption: [Overview of areas where the presence of the species is certain according to the CP model under a risk level $alpha = 0.05$ (A). The certain areas are in dark green, and the uncertain areas, wherein both presence and absence are credible, are in dark grey. (B) Surface covered by the sure absence and total range (including the superficy of the unsure area) for different risk levels #add[(expressed as the desired confidence, $1 - alpha$)]. Note that for $alpha approx 0.1$, the total predicted range starts being lower than the range predicted by the SDM, and the uncertain range collapses. (C) Distribution of variability from #ref(<predictions>)B by type of CP model outcome #add[under $alpha = 0.05$].],
  placement: auto
) <uncertainty>

In #ref(<uncertainty>), we show that this prediction indeed stands: the range as predicted by the SDM (#ref(<uncertainty>, supplement: "fig.")A) falls within the range of unsure predictions. We also see that lowering the risk level $alpha$ leads to a contraction of the area (in $"km"^2$) considered to be credibly associated to only the presence of the species ($C = {+}$), while the range that is ambiguous ($C = {+, -}$) increases (#ref(<uncertainty>)B). As far as ecologists are concerned, the areas in which the #change[prediction set] only has a score for the absence of the species are the easiest to make sense of: they correspond to regions where the model is certain (under the specified risk level) that the species is absent. All other areas (assuming that there are no predictions for which the #change[prediction set] is empty, which I discuss in the next section) are _potentially_ part of the range of the species: some certainly #add[(would have been included in the non-conformal prediction)], some uncertainly #add[(would not have been included in the non-conformal prediction)].

#cut[Depending on the purpose for which the SDM is produced, the uncertain areas can be treated differently. As #cite(<prescott2025performance>, form: "prose") argue, when dealing with invasive species, it may be more reasonable to interpret SDMs by erring on the side of caution, which here would mean considering that unsure presence area should be considered part of the species's range. On the other hand, when SDMs are meant to guide conservation actions that are costly or should be focused on areas of high certainty of suitability for the target species #cite(<peknicova2016application>), it may make sense to ignore the unsure presences. ]
...
== Conformal prediction #change[under climate projection]

#add[
=== Certain and uncertain range shifts
]

In a recent contribution, #cite(<smith2025linking>, form: "prose") suggest that because of issues around the use of thresholds, projections of SDMs under climate change scenarios may benefit from a more continuous perspective. In this section, I present a comparison of the conformal prediction of the range under a climate change scenario (SSP370. 2081-2100), to illustrate how the future conformal range can convey information about the certainty of some types of range shift. These results are presented in #ref(<gainloss>).

#figure(
  image("figures/transitions.png", width: 100%),
  caption: [Overview of the conformal prediction of the range for the future climate data, equivalent to #change[#ref(<uncertainty>, supplement: "fig.")A] (panel A). #change[Sankey diagram for the transitions between absent, unsure, and present predictions for the current (left) and future (right) bioclimatic variables (panel B).] #add[The colors in panels A and B are the same.]],
  placement: auto
) <gainloss>
...
Conformal prediction, like most SDM methods, is not quite delivering a true estimate of the probability of presence #cite(<phillips2013estimating>). Nevertheless, it brings valuable information, in the form of a quantified measure of whether a prediction comes with uncertainty (are both presence and absence in the #change[prediction set]?) in a way that is directly comparable with the non-conformal prediction. "Class overlap", where both presences and absences are observed under the same values of the predictions, decreases the predictive performance of models #cite(<valavi2022predictive>) --- CP is naturally suited at handling this, by assigning the area where overlap occurs to uncertain predictions.

#add[A useful categorization of uncertainty is to differentiate between its aleatoric and epistemic component. #cite(<mansfield2025epistemic>, form: "prose"), for climate prediction models, suggest that aleatoric uncertainty stems from the variability in input data, whereas epistemic uncertainty stems from an unability to identify the parameters that unambiguously map an input to a model prediction. The same idea has been suggetsed for ecological dynamics models #cite(<reimer2022uncertainty>). #cite(<sale2025aleatoric>, form: "prose") recently suggested that CP could capture both forms of uncertainty, although primarily because the disentanglement of epistemic _v._ aleatoric uncertainties is a difficult task, especially under climate change #cite(<kujala2013treatment>). Under this perspective, CP could serve as a mapping of the aggregate uncertainty for a given prediction problem.]

#add[
== Increasing the relevance of CP to species distribution modeling

#cite(<davis2024reproducible>, form: "prose") previously suggested using CP to approximate a confidence interval around a probability of species presence, which considers species prediction as, fundamentally, a regression problem. As SDMs are more traditionally viewed as classification problems, a proper accounting of the method for CP for classification is required in order to understand what future research efforts should focus on. This is particularly important as alternative frameworks around CP, like Adaptive Conformal Inference  #cite(<szabadvary2026conformal>), are emerging: the ontologic status of SDM as a machine learning practice must be clear.

Although the change in climatic conditions has been measured through climate velocity #cite(<brito-morales2018climate>), measures of climate _novelty_ are likely to be more informative for the interpretation of CP. Exchangeability of the data is a core assumption of CP, and although some recent evidence suggests that CP is relatively robust to violations of this assumption (they have been discussed in earlier sections of this manuscript), a very high novelty is likely to result in locally non-exchangeable data: the model would be applied (and its uncertainty quantified) on data points that are outside of the (joint) distribution of variables in the training set. Beyond climatic novelty, measurement of potential covariate shift between the training dataset and _both_ the current and future climate conditions, may provide a clearer understanding of where and when predictions are likely to be more uncertain.
]

#add[
== CP as a tool for effective and transparent communication
]
