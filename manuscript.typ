...
The value of $accent(q, hat)$ can be obtained either through using a holdout set for training (Split Conformal Prediction), #add[using adaptive prediction sets TK agelopoulos bates 2023], by retraining the model in a way aking to Leave-One-Out cross-validation (Full Conformal Prediction), through the use of quantile regression #cite(<romano2019conformalized>), or through taking the median of several estimates of $accent(q, hat)$ after cross-validation #cite(<vovk2018crossconformal>). #cut[In this manuscript, I employ the later method, as it provides a rapid and statistically acceptable estimate of $accent(q, hat)$, without requiring too much computing time.]
