...
A common way to capture information about the variability of SDMs is to rely on non-parametric bootstrapping #cite(<Valavi2021>), wherein models trained on random subsets of the data are compared to estimate the distribution of the response under incomplete sampling. This approach captures more than one type of variability #cite(<Thuiller2019>), and provide valuable information about the range of performances that can be expected from a model. Other methods are built into the predictor itself, as is the case for _e.g._ BARTs #cite(<Carlson2020>), which estimate their own uncertainty. But either situation comes with drawbacks. Bootstrapping requires to train and evaluate the model hundreds of times, and on partial datasets, which is computationally inefficient. Using #change[methods that are specific to a particular classification algorithm] limits one to the classifier for which these methods are available, which prevents for example the use of a new algorithm with the same estimation of uncertainty.
...
There are several ways to decide whether a point prediction from the model results in which #change[prediction set]. A core assumption of CP is that the data used for training should be exchangeable, or in other words, their joint probability distribution should be (close to) invariant under finite permutations #cite(<Aldous1985>). This will almost never be the case for data with a spatial structure; nevertheless, this does not rule out the use of CP for species distribution modeling, as #cite(<Oliveira2024>, form: "prose") show that CP is acceptably robust to lack of exchangeability. #add[The purpose of this section is to establish a general overview of how conformal predictions are obtained, and some of the multiple variations that exist will be introduced throghout the text.]
...
The value of $accent(q, hat)$ can be obtained either through using a holdout set for training (Split Conformal Prediction), #add[using adaptive prediction sets TK agelopoulos bates 2023], by retraining the model in a way aking to Leave-One-Out cross-validation (Full Conformal Prediction), through the use of quantile regression #cite(<Romano2019>), or through taking the median of several estimates of $accent(q, hat)$ after cross-validation #cite(<Vovk2018>). #cut[In this manuscript, I employ the later method, as it provides a rapid and statistically acceptable estimate of $accent(q, hat)$, without requiring too much computing time.]
...
In the rest of this analysis, I will set $alpha = 0.05$. As noted by #cite(<Angelopoulos2023a>, form: "prose"), this corresponds to estimating whether a specific prediction falls within, or outside of, the 95% confidence interval across all predictions, which is a convenient callback to frequentist statistics' usual risk tolerance. Recall that the CP #change[prediction sets] are estimated based on the model output, and therefore even when aiming for full coverage, there may be non-ambiguous combinations of environmental predictors.
...
BUT - similar issue plagues all transfer of SDMs in future climate projections, chances of covariate shift usually not handled beyond measuring novelty - therefore assumed that covaraite shoift is minimal

`allen2025insample` - in-sample calibration is enough to get the guarantees required for conformal prediction

`balinsky2025when` - can re-use calibration set, gives ok coverage

`barber2023conformal`- methods to deal with it
